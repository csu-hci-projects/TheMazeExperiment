...
class AllocentricPlayerController : PlayerController
{
    ...
    {
        if (PersistentManager.Instance.currentLocation != PersistentManager.Instance.previousLocation)
        {
            PersistentManager.Instance.previousLocation = PersistentManager.Instance.currentLocation;
        }
    ...
    IEnumerator MovetoIntersection(System.Action<Vector3> callback)
    {
        ...
        PersistentManager.Instance.currentLocation = new int[] { (int)Math.Round(moveVector[0]), (int)Math.Round(moveVector[2]) };
