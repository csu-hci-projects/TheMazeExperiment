...
public class PlayerController : MonoBehaviour
{
    ...
    public MainMenu referenceScript;
    ...
    void Update()
    {
        ...
        if (allowInput) {
            ...
            if ((Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.LeftArrow)))
            {
                ...
                StartCoroutine(Rotate90(Vector3.up * -90, 0.2f));
            }
            // Rotate right
        ...
        if (movingToTarget == true)
        {
            ...
            {
                // Move to new vector
                transform.position = Vector3.MoveTowards(transform.position, toPosition, Sanic * Time.deltaTime);
                // Player reaches new vector
                if (NearlyEqual(transform.position.x, toPosition.x, 0.1f))
                {
                    // No longer moving, and allow input again
                    movingToTarget = false;
                    allowInput = true;
                    if (mazeArray[playerPosition[0], playerPosition[1]] == "0F")
                    {
                        referenceScript.ChangeScene();
                    }
                }
            }
        ...
        else
        {
            ...
            transform.position = Vector3.MoveTowards(transform.position, move, Sanic * Time.deltaTime);
        }
    }
    ...
    string[,] ReadMazeFile()
    {
        ...
        string[] lines = File.ReadAllLines(Directory.GetCurrentDirectory() + "/Assets/Mazes/ConditionA/" + file);
